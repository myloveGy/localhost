 ...
 * @param int  $options
 */
function echoJson($mixed, $isExit = true, $options = JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)
{
...
{
    echoJson(['status' => 'fail', 'code' => $code, 'msg' => $msg]);
}
 ...
 * @return array
 */
function curlRequest($url, $params = [], $method = 'POST', $options = [], $json = false)
{
    // 如果是GET请求，那么需要追加参数
    ...
    if (curl_errno($curl)) {
        ...
        $ok   = true;
        $data = $json ? json_decode($data, true) : $data;
    }
...
function curlPost($url, $params, $options = [], $json = true)
{
    return curlRequest($url, $params, 'POST', $options, $json);
}

function curlGet($url, $params, $options = [], $json = false)
{
    return curlRequest($url, $params, 'GET', $options, $json);
}
